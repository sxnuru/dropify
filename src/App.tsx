...
import {
  ...
  ChevronDown, Menu, ArrowRight, Star, Tag, Award, ShieldCheck, Mail, Phone, MapPin, Compass,
  RotateCcw, Lock, CreditCard, Gift, Truck, Package, LogOut
} from 'lucide-react';

const megaMenuColumns = [
  {
    name: 'Apparel & Recreation',
    icon: <Compass className="w-3.5 h-3.5 text-blue-500" />,
    items: [
      { name: 'Fashion', desc: 'Curated apparel, headwear, and garments' },
      { name: 'Sports & Outdoors', desc: 'Athletic gear, sports equipment, and accessories' }
    ]
  },
  {
    name: 'Tech & Play',
    ...
    items: [
      { name: 'Electronics', desc: 'Tomorrow tech, power accessories, and devices' },
      { name: 'Toys & Games', desc: 'Retro play, board games, and action figures' }
    ]
  },
  {
    ...
    items: [
      { name: 'Home & Garden', desc: 'Spatial accessories, decor, and living items' },
      { name: 'Health & Personal Care', desc: 'Daily wellness, first aid, and self-care items' }
    ]
  },
  {
    name: 'Pantry & Miscellaneous',
    icon: <Sparkles className="w-3.5 h-3.5 text-blue-500" />,
    items: [
      { name: 'Food & Grocery', desc: 'Curated pantry, snacks, and organic goods' },
      { name: 'Other', desc: 'Uncategorized boutique items and unique finds' }
    ]
  }
];
...
export default function App() {
  ...
  const handleUpdateProfile = (updatedUser: { fullName: string; email: string; phone: string }) => {
    setCurrentUser(prev => {
      ...
      console.log("[handleUpdateProfile] setCurrentUser updated profile:");
      console.log("- fullName:", nextUser.fullName);
      console.log("- email:", nextUser.email);
      console.log("- role:", nextUser.role);
      return nextUser;
    });
  };
    ...
    <div className="min-h-screen bg-[#FCFCFC] flex flex-col justify-between">
      ...
      <header className="sticky top-0 z-40 bg-[#FCFCFC]/90 backdrop-blur-md border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between gap-4 relative">
            ...
            >
              ...
              {isMegaMenuOpen && (
                <div
                  ...
                  className="absolute left-0 right-0 top-full w-full bg-white/98 backdrop-blur-md border-b border-slate-200/50 shadow-2xl p-8 z-50 animate-fadeInDown grid grid-cols-1 md:grid-cols-4 gap-6 rounded-b-3xl border-t border-slate-100"
                >
          ...
          <div className="flex items-center gap-3 shrink-0">
              ...
              {isSearchFocused && (
                ...
                >
                  <div className="flex justify-between items-center border-b border-slate-100 pb-2">
                    <span className="font-mono text-[9px] text-slate-400 font-bold uppercase tracking-widest">Search Console</span>
                    <button
                      onClick={() => setIsSearchFocused(false)}
                      className="text-[10px] font-mono text-slate-500 hover:text-slate-950 font-bold uppercase tracking-wider flex items-center gap-1 cursor-pointer"
                    >
                      ✕ Close
                    </button>
                  </div>
                  ...
                  {!searchQuery.trim() ? (
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          {['Headbands', 'Sun Hat', 'Swimming', 'Basketball', 'Resistance Trainer'].map((term, termIdx) => (
                            <button
                              key={`${term}-${termIdx}`}
                      ...
                      <div className="space-y-2 border-t border-slate-100 pt-3">
                        ...
                        <div className="grid grid-cols-2 gap-1.5">
                          {[
                            ...
                            { name: 'Home & Garden', label: 'Home & Garden' },
                            { name: 'Health & Personal Care', label: 'Wellness Sector' }
                          ].map((col, colIdx) => (
                            <button
                              key={`${col.name}-${colIdx}`}
                              onClick={() => {
            ...
            <div className="relative">
              <button
                ...
                className={`p-2.5 rounded-xl transition-all flex items-center justify-center cursor-pointer ${currentTab === 'account' ? 'bg-blue-50 text-blue-600' : 'bg-slate-50 hover:bg-slate-100 text-slate-600 hover:text-slate-950'
                  }`}
                name="Client Profile Dashboard"
              >
          ...
          {/* Old Mega Menu removed (now nested in trigger) */}
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 flex-1 w-full">
        ...
        {currentTab === 'seller-hub' && currentUser?.role === 'admin' && (
          <SellerDashboard
            onAddProduct={handleAddProduct}
            onEditProduct={handleEditProduct}
            onDeleteProduct={handleDeleteProduct}
        ...
        {currentTab === 'auth' && (
          <AuthPages
            onSuccess={() => {
              console.log("[AuthPages onSuccess] Login/Signup succeeded. Relying solely on onAuthStateChanged to populate currentUser.");
              setCurrentTab('account');
            }}
                        ...
                        <div className="text-right flex flex-col justify-between items-end h-full min-h-[56px]">
                          ...
                          <div className="flex gap-2 mt-2">
                            <button
                              ...
                              name="Save this piece for later browsing"
                            >
                              Save for Later
                            </button>
                            <button
                ...
                <div className="space-y-4 animate-fadeIn">
                  ...
                  <div className="space-y-2.5 pt-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5">
                      {[
                        { id: 'standard', name: 'Standard Ground', desc: '2-5 business days', price: cartSubtotal >= 100 ? "COMPLIMENTARY" : "+£4.99" },
                        { id: 'express', name: 'Express Courier', desc: '2 business days', price: cartSubtotal >= 100 ? "+£2.50" : "+£7.49" },
                        { id: 'overnight', name: 'Next-Day Delivery', desc: 'Next business morning', price: cartSubtotal >= 100 ? "+£5.00" : "+£9.99" }
                      ].map((tier) => (
                        <div
                          key={tier.id}
                  ...
                  <div className="pt-2">
                    <button
                      ...
                      className="w-full py-3 bg-slate-900 text-white hover:bg-blue-600 disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed text-xs font-sans font-bold rounded-xl transition-all shadow-md uppercase tracking-wider cursor-pointer"
                    >
          ...
          <div className="space-y-3">
            ...
            <ul className="space-y-1.5 text-slate-400 font-medium">
              ...
              {currentUser?.role === 'admin' && (
                <li><button onClick={() => setCurrentTab('seller-hub')} className="hover:text-white transition-colors text-left block w-full cursor-pointer">Seller Portal</button></li>
              )}
            </ul>
          </div>
