...
import { Star, Heart, Eye, ShoppingCart, Truck, AlertTriangle } from 'lucide-react';
...
const ProductCard = React.memo(function ProductCard({
  ...
  const handleAddToCartClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    onAddToCart(product, product.colors[0] || 'Default', product.sizes[0] || 'Default');
    setIsAdded(true);
    setTimeout(() => setIsAdded(false), 2000);
  };

  const hasDiscount = product.discountPercent && product.discountPercent > 0;
  const originalPrice = product.originalPrice || (hasDiscount ? Math.round(product.price / (1 - (product.discountPercent || 0) / 100)) : undefined);
  ...
  return (
    ...
    >
      <div className="space-y-4">
        {/* Aspect Ratio 1:1 Image Frame with rich overlays */}
        <div className="relative aspect-square rounded-2xl overflow-hidden bg-slate-50/50 flex items-center justify-center">
          ...
          {/* Floated badging system top-left */}
          ...
          {/* Premium Wishlist Button inside the top-right aspect zone */}
          <button
          ...
          {/* Interactive slide-up specification pill or direct action layer on hover */}
        ...
        {/* Detailed Meta Segment */}
        <div className="space-y-1.5 text-xs">
          {/* Brand Row + Star score rating */}
          <div className="flex justify-between items-center text-[10px] font-mono">
            <span className="uppercase font-bold tracking-wider text-slate-400">{product.brand}</span>
      ...
      {/* Bottom Segment: Price, Transit and Dual Action buttons */}
      <div className="pt-3.5 mt-3.5 border-t border-slate-100 font-sans text-xs flex flex-col gap-2 shrink-0">
        
        {/* Elegant Free Transit Row */}
        <div className="flex items-center gap-1 text-[10px] text-slate-400/95 font-medium leading-none">
          <Truck className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
          <span>{product.estimatedDelivery || "Complimentary 3-day transit"}</span>
        </div>

        {/* Pricing tag + Action Bar */}
        <div className="flex items-center justify-between mt-1">
          <div className="flex flex-col">
            <span className="font-mono font-bold text-slate-950 text-sm leading-none">{formatPrice(product.price)}</span>
            {originalPrice && originalPrice > product.price && (
              <span className="text-slate-400 line-through text-[10px] mt-1 block leading-none font-mono">
          ...
          <div className="flex gap-1.5">
            <button
              ...
              title="Inspect specifications & details"
            >
            ...
            <button
              ...
              title={isOutOfStock ? "Out of stock" : "Add piece to shopping bag"}
            >
              {isAdded ? (
                <span className="text-[10px] font-mono font-bold px-1 animate-fadeIn uppercase">ADDED</span>
              ) : (
